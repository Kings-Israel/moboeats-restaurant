        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
          ...
          <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 sm:mb-0">
              <h1 class="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">Groceries ✨</h1>
            </div>

            <!-- Right: Actions  -->
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
              <button class="bg-emerald-600 p-2 px-4 col-span-1 text-slate-50 font-semibold rounded-full justify-end" @click="exportMenus()">Export Groceries</button>
              <button class="bg-[#2E5945] hover:bg-indigo-500 p-2 px-3 text-white rounded-full font-semibold" @click="addMenuModal = true">Add Grocery</button>
              <modal-action :id="'addMenu'" :modal-open="addMenuModal" @close-modal="addMenuModal = false" :add-class="'max-w-4xl'">
                <!-- Add/Edit Menu -->
                <form class="flex flex-col justify-around p-3 px-4" @submit.prevent="createMenu">
                  <div class="grid grid-cols-2 gap-2">
                    <div class="space-y-4">
                      <div>
                        <label class="block text-sm font-medium mb-1" for="title">Name / Title</label>
                    ...
                    <div class="space-y-4">
                      ...
                      <div>
                        <label class="block text-sm font-medium mb-1" for="price">Preparation Time (in minutes)</label>
                        <input id="title" class="form-input w-full rounded-lg" type="number" min="0" v-model="addMenuPreparationTime" />
                      </div>
                      ...
                      <div>
                        <label class="block text-sm font-medium mb-1" for="categories">Branches <i class="fas fa-question-circle text-sm hover:cursor-pointer" data-title="Specific branches where the menu will appear"></i></label>
                  ...
                  <div class="flex justify-end bottom-2 py-2">
                      ...
                      <tr v-for="menu_item in menu" :key="menu_item.id" :class="menu_item.id === editMenuId ? 'bg-slate-600 transition duration-200 ease-in-out' : 'transition duration-300 ease-in-out'">
                        <td class="p-2">
                          <div class="flex items-center">
                            ...
                            <div class="text-slate-800 ml-2">{{ menu_item.title }}</div>
                          </div>
                        </td>
                        <td class="p-2">
                          <router-link :to="{name: 'restaurant-details', params: { id: menu_item.restaurant.uuid}}" class="text-sky-800 hover:text-sky-500 font-bold">
                        ...
                        <td class="p-2">
                          <div class="text-sky-700 font-semibold">{{ menu_item.preparation_time }} mins</div>
                        </td>
                        <td class="p-2">
                          <div class="text-center text-sky-700 font-semibold">{{ menu_item.order_items_count }}</div>
                        </td>
                        <td class="p-2">
                          <div v-if="menu_item.created_at" class="text-center text-sky-700 font-semibold">{{ moment(menu_item.created_at).format('Do MMM Y') }}</div>
                        </td>
...
export default {
  ...
  setup() {
    ...
    const getMenu = () => {
      $http.get(`/restaurant/groceries`)
        .then(response => {
          ...
          addMenuCategories.value.push(category_id)
    ...
    const exportMenus = () => {
      $http.get('/restaurant/groceries/export/data?search='+menuSearch.value,
          {
    ...
    watch(menuSearch, async (newSearch, oldSearch) => {
      $http.get('/restaurant/groceries?search='+newSearch)
        .then(response => {
          menu.value = []
          menu.value = response.data.data.menu.data
          categories.value = response.data.data.categories
