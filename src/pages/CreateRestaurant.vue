    ...
    <div class="">
      ...
      <div class="w-full">
        <div class="min-h-[100dvh] h-full flex flex-col after:flex-1">
          <div class="flex-1">
            <div
              class="flex items-center justify-center h-24 px-4 sm:px-6 lg:px-8"
            >
              <!-- Logo -->
          ...
          <div class="rounded-md lg:px-4 py-2 bg-gray-100 mx-4 lg:mx-32">
            <h1
              v-if="status == 'update'"
              class="px-2 lg:px-0 text-xl text-slate-800 dark:text-slate-100 font-bold mb-6"
            >
              Edit Details ✨
            </h1>
            <h1
            ...
            <h3 class="px-2 lg:px-0 text-xl text-slate-700 font-bold my-2">
              Partner Details
            </h3>
            <!-- Form -->
                ...
                <div class="w-full lg:w-1/2 space-y-4">
                  <div class="flex gap-2">
                    <label
                      class="block text-sm font-medium mb-1"
                      for="restaurant_name"
                      >Do you offer dine-in services?</label
                    >
                  ...
                  <div>
                    <label class="block text-sm font-medium mb-1" for="email"
                      >Have a website? Enter URL</label
                    >
                    <input
                      id="email"
                      class="w-full rounded-md border-2 border-slate-400 text-slate-900"
          ...
          <div class="flex items-center justify-end mt-6 mx-4 lg:mx-32 my-2">
            <button
              class="btn bg-red-500 hover:bg-red-600 text-white ml-3 whitespace-nowrap"
              type="button"
              @click="logout"
            >
              Logout
...
export default {
  ...
  setup() {
    ...
    const onSubmit = () => {
      ...
      submit_button.value.innerHTML =
        status.value == "create" ? "Creating..." : "Updating...";
      submit_button.value.setAttribute("disabled", "disabled");

      if (status.value == "create") {
        $http
          .post("/restaurant/restaurant", formData)
          .then((response) => {
        ...
        $http
          .post(
            `/restaurant/restaurants/${restaurant.value.uuid}/update`,
            formData
          )
          .then((response) => {
            router.push({
              name: "restaurant-details",
              params: { id: restaurant.value.uuid },
            });
    ...
    onMounted(() => {
      if (route.params.id) {
        $http
          .get("/restaurant/restaurants/" + route.params.id)
          .then((response) => {
            ...
            sitting_capacity.value =
              restaurant.value.attributes.sitting_capacity;
            ...
            if (
              restaurant.value.attributes.latitude &&
              restaurant.value.attributes.longitude
            ) {
              center.value = {
