        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
          ...
          <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 sm:mb-0 col-span-2">
              <h1 class="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">Dine In Orders ✨</h1>
            </div>

            <!-- Action Buttons -->
            <button class="bg-emerald-600 px-4 py-1 col-span-1 text-slate-50 font-semibold rounded-full justify-end w-60" @click="exportOrders()">Export Dine In Orders</button>
          </div>
          <div class="grid md:grid-cols-5 gap-2 mb-2">
            ...
            <button class="bg-red-500 text-white rounded-full col-span-1 w-full h-fit py-1 my-auto" @click="search = '', from_created_at = '', to_created_at = ''">Clear Fields</button>
          </div>
                ...
                <table class="table-auto w-full dark:text-slate-300">
                  ...
                  <thead class="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/20 border-t border-b border-slate-200 dark:border-slate-700">
                    <tr>
                      ...
                      <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="font-semibold text-left">Dine In Date</div>
                      </th>
                      <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                  ...
                  <tbody class="text-sm divide-y bg-slate-50 divide-slate-200 dark:divide-slate-700">
                    <tr v-for="order in orders" :key="order.id">
                      ...
                      <!-- <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="text-left text-slate-700">{{order.restaurant.attributes.address}}</div>
                      </td> -->
                      <!-- <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="text-left text-slate-700">{{moment(order.created_at).format('D MMM Y')}}</div>
                      </td> -->
                      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="text-left text-slate-700" v-if="order.dine_in_date">{{moment(order.dine_in_date).format('D MMM Y H:m A')}}</div>
                        <div class="text-left text-slate-700" v-else>Not Set</div>
                      </td>
...
export default {
  ...
  setup() {
    ...
    const exportOrders = () => {
      $http.get('/restaurant/orders/export/data?delivery=false&search='+search.value+'&from_created_at='+from_created_at.value+'&to_created_at='+to_created_at.value,
          {
    ...
    const getOrders = () => {
      $http.get('/restaurant/orders?delivery=false')
        .then(response => {
          orders.value = []
          pagesLinks.value = []
          ...
          response.data.data.forEach(order => {
            orders.value.push({
              ...
              dine_in_date: order.attributes.booking_time,
              seat_number: order.relationships.reservation.seat_number
            })
          })
        })
    }
    ...
    watch(search, async (newSearch, oldQuestion) => {
      $http.get('/restaurant/orders?delivery=false&search='+newSearch)
        .then(response => {
          nextPageUrl.value = response.data.links.next
          lastPageUrl.value = response.data.links.last
          prevPageUrl.value = response.data.links.prev
    ...
    watch(from_created_at, async (newSearch, oldQuestion) => {
      $http.get('/restaurant/orders?delivery=false&from_booking_date='+newSearch)
        .then(response => {
          nextPageUrl.value = response.data.links.next
          lastPageUrl.value = response.data.links.last
          prevPageUrl.value = response.data.links.prev
    ...
    watch(to_created_at, async (newSearch, oldQuestion) => {
      $http.get('/restaurant/orders?delivery=false&to_booking_date='+newSearch)
        .then(response => {
          nextPageUrl.value = response.data.links.next
          lastPageUrl.value = response.data.links.last
          prevPageUrl.value = response.data.links.prev
