    ...
    <div v-show="modalOpen" :id="id" :class="'fixed inset-0 z-50 overflow-hidden flex items-center my-4 justify-center px-4 sm:px-6'" role="dialog" aria-modal="true">
      <div ref="modalContent" :class="'bg-white dark:bg-slate-800 rounded shadow-lg overflow-auto w-full max-h-full '+addClass">
        <div class="p-6">
          <div class="relative">
            <!-- Close button -->
            <button type="button" class="absolute top-0 right-0 text-slate-400 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-400" @click="$emit('close-modal')">
